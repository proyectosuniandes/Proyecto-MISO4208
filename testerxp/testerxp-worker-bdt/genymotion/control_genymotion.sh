...
stop_all_genymotion() {
  ...
  ps aux | grep -i './genymotion/player --vm-name' | awk '{print $2}' | xargs kill || echo "No simulators were running."

  # stop ADB to get rid of stuck devices
  ./genymotion/tools/adb kill-server
}
...
# returns ./tools/adb serial number from device name, only returns first match
get_adb_serial_from_name() {
  DNAME="$1"
  # shellcheck disable=SC2005
  echo "$(./genymotion/tools/adb devices -l | grep "${DNAME//[-. ]/_}" | head -1 | cut -d\  -f1)"
}
...
is_adb_running() {
  if ! ./genymotion/tools/adb devices | grep -q "$1"
    then
      return 1
  fi
}
...
is_booted() {
  ...
  BOOTED="$(./genymotion/tools/adb -s $SERIAL shell getprop init.svc.bootanim)"
  if [[ ! "$BOOTED" == *"stopped"* ]]
    then
