...
CREATE TYPE IF NOT EXISTS tipo_app AS ENUM ('movil', 'web');
CREATE TYPE IF NOT EXISTS tipo_prueba AS ENUM ('E2E', 'random','BDT','VRT');
CREATE TYPE IF NOT EXISTS modo_prueba AS ENUM ('headless','headful');
CREATE TYPE IF NOT EXISTS estado_ejecucion AS ENUM ('registrado','ejecutado','pendiente');
...
CREATE TABLE IF NOT EXISTS prueba(
  ...
  CONSTRAINT id_version_fkey FOREIGN KEY (id_version)
    REFERENCES version (id_version) MATCH SIMPLE
    ON UPDATE NO ACTION ON DELETE NO ACTION,
  CONSTRAINT id_app_fkey FOREIGN KEY (id_app)
    REFERENCES app (id_app) MATCH SIMPLE
    ON UPDATE NO ACTION ON DELETE NO ACTION
